...
const LOCAL_CONFIG = join(process.cwd(), "../../config/admin.local.env");
...
try {
  execFileSync(process.execPath, [wranglerCli, "d1", "execute", "462019-portfolio", command, "--file", sqlFile], { stdio: "inherit" });
  clearLocalPassword();
  console.log("Administrator synchronized. The local plaintext password has been cleared.");
} finally {
...
function readLocalConfiguration(): { username: string; password: string; accessEmail: string; pepper: string } {
  const entries = Object.fromEntries(readFileSync(LOCAL_CONFIG, "utf8").split(/\r?\n/).filter(Boolean).map((line) => line.split(/=(.*)/s).map((value) => value.trim())));
  ...
  // Prefer an ephemeral environment value so the production Pepper never needs to be written to disk.
  const pepper = process.env.AUTH_PEPPER ?? entries.AUTH_PEPPER;
  if (!username?.match(/^[a-z0-9_]{3,32}$/) || !password || password.length < 12 || !accessEmail || !pepper) throw new Error("Fill the administrator fields and provide AUTH_PEPPER through the environment or config/admin.local.env.");
  return { username, password, accessEmail, pepper };
}

/** Remove the reusable plaintext secret only after the database transaction succeeds. */
function clearLocalPassword(): void {
  const source = readFileSync(LOCAL_CONFIG, "utf8");
  writeFileSync(LOCAL_CONFIG, source.replace(/^ADMIN_PASSWORD=.*$/m, "ADMIN_PASSWORD="), { encoding: "utf8", mode: 0o600 });
}
